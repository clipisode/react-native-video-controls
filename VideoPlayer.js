...
export default class VideoPlayer extends Component {
  ...
  _togglePlayPause() {
    ...
    if (state.paused) {
      typeof this.events.onPause === "function" &&
        this.events.onPause({ duration, currentTime });
    } else {
      typeof this.events.onPlay === "function" &&
        this.events.onPlay({ duration, currentTime });
    }
    ...
    this.player.seekPanResponder = PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (time >= this.state.duration && !this.state.loading) {
          ...
          this.seekTo(time);
          if (this.onSeek)
            this.onSeek({ duration: this.state.duration, currentTime: time });
          this.setControlTimeout();
